// src/pages/api/token/[address].ts
...
function toFloat(v: any, decimals: number | null | undefined) {
  try {
    ...
    const d = Math.max(0, Number(decimals ?? 18)); // <<— IMPORTANT: Number(...) pour éviter null
    const s = String(v);
    if (/^\d+$/.test(s)) {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!r.ok) {
      return res.status(r.status).json({ error: `explorer ${r.status}` });
    }
    ...
    const total = toFloat(t.total_supply, decimals);
    // certains explorers n’exposent pas circ: fallback sur total
    const circ = toFloat(t.circulating_supply ?? t.total_supply, decimals);
    const holders =
      t.holders_count ?? t.holder_count ?? t.holders ?? null;
